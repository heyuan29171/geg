...
function rogueUpgradesHTML() {
  ...
  return '<div class="rogue-upgrades">' +
    '<div class="rogue-upg-head">局外养成 <span class="hint">金币余额 <b class="rogue-coins">' + (S.coins || 0).toLocaleString() + '</b></span></div>' +
    rows + '</div>';
}
...
function rogueBestHTML() {
  ...
  return '<div class="panel"><h3>历史最高战绩</h3>' +
    ...
    '<div class="kv"><span>结算</span><b>+' + b.frags.toLocaleString() + ' 碎片 · +' + b.coins.toLocaleString() + ' 金币</b></div>' +
...
function rogueResultHTML() {
  ...
  return '<div class="panel">' +
    ...
    (r.penalty > 0 ? '<div class="kv"><span>未达 ' + r.goal + ' 波目标</span><b class="miss-text">-' + r.penalty.toLocaleString() + ' 金币</b></div>' : '') +
    '<div class="kv"><span>金币收益</span><b>+' + r.coins.toLocaleString() + '</b></div>' +
...
function rogueUpdateDyn() {
  ...
  const bankTxt = '金币 <b>' + R.bank.toLocaleString() + '</b>（本波 +' + rogueWaveCoins(R.wave) + '）· 已获基础碎片 <b>' + rogueRunFrags().toLocaleString() + '</b>';
